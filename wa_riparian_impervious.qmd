...
In Washington, high resolution change detection (HRCD) processing of aerial imagery at a 1m scale (Pierce 2015) enables evaluation of impervious cover that addresses these considerations. The HRCD approach combines automated delineation of change polygons with extensive manual review and verification against before-after image pairs to generate a detailed dataset of land cover alteration. We used these data in conjunction with a statewide polygon tessellation of riparian corridors that is grounded in current policy guidelines, and we compare the resulting estimates with those derived from independent public land cover data in a fixed-width corridor on a different hydrography. We describe these estimated increases in imperviousness relative to salmon distribution within river networks as well as concurrent changes in human population and housing, before discussing policy implications and opportunities for improved ongoing assessment.



[*expand this paragraph here? or bring into methods/discussion with outputs from DL?*]
...
tibble(
  Characteristic = c(
    "Hydrography","Buffer dimension", "Land cover resolution","Years",
    "Assessed riparian corridor",
    "Estimated annual impervious increase"),
  `RDE-HRCD` = c(
    "NHD (2022)", "SPTH~200~, varied (approx. 30-75m),", "1m", "2011-2017", 
    round(wa_imp$reach_ha[4]), 
    paste0("impv: ",round(wa_imp$impv_ha_y[4]), ", semi:", round(wa_imp$semi_ha_y[4]))
    ),
  `StreamCat-NLCD` = c(
    "NHDPlus v2.1 1:100K", "100m, fixed", "30m", "2001-2019", 
    100*round(sum(filter(sc_nlcd, nlcd_year==2011)$catareasqkmrp100))
    ,
    sc_nlcd_year_sum |>
      select(-ac) |> 
