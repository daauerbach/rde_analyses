...
This analysis examines the intersection of these high resolution changes with a statewide polygon tessellation of riparian corridors derived from the SPTH200 standard. We describe landscape scale patterns of impervious increases and consider concurrent changes in human population and housing, as well as the positioning of new riparian imperviousness relative to salmon distribution within river networks.
...
list(
  ...
  reach_ac = sf_cnty_rde |>  
    ...
    scale_fill_gradient(name = "hectares", labels = scales::comma, low = "#BEE7DF", high = "#212540", na.value = "pink") +
...
During the 2011-2017 period in these HRCD data, `r round(wa_imp$impv_inc_ha[4])` hectares of new impervious cover were estimated to have been added to the riparian corridor (table 1; `r round(wa_imp$impv_inc_ac[4])` acres), corresponding to an annual increase of `r round(wa_imp$ha_y[4])` ha/year (`r round(wa_imp$ac_y[4])` acres/year). 

Although total additional impervious area was similar between non-salmon and Chinook-associated reaches, the increase was disproportionately greater in reaches associated with Chinook distribution, with new impervious making up an order of magnitude higher percentage of the total assessed area (0.1% vs 0.01%, table 1, fig NNN).
...
# }/{
  rde_cnty_swifd |> 
    ...
    scale_fill_manual(values = pal_swifd_chin) +
    labs(x = "SWIFD status", y = "Acres impervious increase per 100 reach ac.") 
#}
...
rde_cnty |> 
  ...
  labs(y = NULL, x = "hectares",
       subtitle = "Per-reach impervious cover increases up to 1 acre")
} + 
  plot_layout(heights = c(1/4,3/4))
```


Total impervious increase varied considerably among county boundary extents, with the largest additions concentrated in more heavily urbanized portions of the state, particularly central and south Puget Sound. Congruent with expectations, changes were greatest in proximity to the large and rapidly growing metropolitan areas of Seattle, Tacoma and Vancouver in western Washington and Spokane and Tri-Cities in the east.

```{r gg_impv_inc_ac_per_100ac_col}

# {
#   {
#   sf_cnty_rde |> 
#     ggplot() + geom_sf(aes(fill = impv_inc_ac), color = "white", show.legend = F) + 
#     scale_fill_gradient(name = "", low = "grey90", high = "grey10") +
#     labs(title = "Riparian impervious cover increase 2011-2017", subtitle = "Sum of HRCD estimates, RMZ+EOW+floodplain")
# }+{
  sf_cnty_rde |> 
    ggplot() + geom_sf(aes(fill = impv_inc_ac_per_100ac), color = "white", show.legend = T) + 
    scale_fill_gradient(low = "grey90", high = "grey10") +
    labs(
      title = "Riparian impervious cover increase 2011-2017", 
      subtitle = "Sum of HRCD estimates, normalized per 100ac RMZ+EOW+floodplain"
      ) +
...
#     ggplot() + 
#     geom_col(aes(
#       ha*impv_inc_ac, fct_reorder(cnty, impv_inc_ac),
#       fill = impv_inc_ac), show.legend = F) + 
#     scale_fill_gradient(name = "hectares", low = "grey90", high = "grey10") +
#     labs(y = NULL, x = "hectares")
# }+{
#   rde_cnty_sum |> 
#     ggplot() +
#     geom_col(aes(
#       impv_inc_ac_per_100ac, fct_reorder(cnty, impv_inc_ac_per_100ac),
#       fill = impv_inc_ac_per_100ac),
#       show.legend = F) + 
#     scale_fill_gradient(name = "", low = "grey90", high = "grey10") +
#     labs(y = NULL, x = "Acres imp. inc. per 100 reach ac.") 
# # }
# } +
...
as_tibble(sf_cnty_rde) |> 
  ...
  geom_col(aes(pct, fct_reorder(cnty, pct, identity), fill = pct)) +
    scale_fill_gradient(name = "reach / cnty", low = grey(0.8), high = grey(0.2)) +
  scale_x_continuous("Proportion of county area", labels = scales::percent) +
  labs(y = NULL)
} +
