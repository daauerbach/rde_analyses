...
tibble(
  ...
  `RDE-HRCD` = c(
    ...
    , 
    paste0(
      "**Total: ", round(wa_imp$reach_ha[4]) |> scales::number(big.mark = ","), "**<br>",
      "RMZ: ", round(ha*wa_imp$rmz_ac[4]) |> scales::number(big.mark = ","), "<br>",
      "EOW: ", round(ha*wa_imp$eow_ac[4]) |> scales::number(big.mark = ","), "<br>",
      "Fld: ", round(ha*wa_imp$flood_ac[4]) |> scales::number(big.mark = ",")
      )
    , 
    paste0(
      paste("Impervious:<br>", round(wa_imp$impv_inc_ha[4]), "ha, ", round(wa_imp$impv_ha_y[4]), "ha/y"),
      "<br>",
      paste("Semi-pervious:<br>", round(wa_imp$semi_inc_ha[4]), "ha, ", round(wa_imp$semi_ha_y[4]), "ha/y")
      )
    ),
  `StreamCat-NLCD` = c(
...
# #direct from FEMA service link in "data sources"
# #not that helpful, not processed to tessellation...
# sf_fld <- st_read("~/Downloads/53053C_20250227/S_FLD_HAZ_AR.shp") |>
#   st_transform(crs = epsg)
# sf_aoi_fld <- sf_fld[sf_aoi,] |> st_crop(sf_aoi)
# sf_aoi_fld |> mapview::mapview(zcol = "FLD_ZONE")
# sf_aoi_fld |> as_tibble() |> count(FLD_ZONE)
 


# #error messages, most likely permissions?
# https://geoserver-riparis.esa-prod.sitkatech.com/geoserver/Riparis/wfs?service=wfs&version=2.0.0&request=GetCapabilities
# https://geoserver-riparis.esa-prod.sitkatech.com/geoserver/Riparis/wfs?service=wfs&version=2.0.0&request=GetFeature&typeNames=Riparis:AllReaches&outputFormat=SHAPE-ZIP&bbox=-122.27395,47.18171,-122.21093,47.26484
...

{
...
}+{
  {
    sf_cnty_rde_sc |>
      ...
      scale_fill_gradient(name = "ha", low = "grey95", high = "#7E4837") + #, limits = c(0,300)
    ...
    sf_cnty_rde_sc |>
      ...
      geom_sf(data = sf_wa_city, color = "brown", show.legend = F) +
      geom_sf_text(data = sf_wa_city, aes(label = name), color = "brown", hjust = 1.1, show.legend = F) +
      scale_fill_gradient(name = "ha", low = "grey95", high = "#483778") +
...
Reflecting the concentration of human population and existing urbanization in western Washington, the relative density of impervious increases was considerably higher in reaches associated with Chinook distribution, despite a similar absolute area of increase to that in non-salmon reaches. 
...
{
wa_imp |> 
  gt(caption = "2011-17 HRCD Impervious Increase within RDE corridor") |> 
...
small percentage of high-proportion-floodplain reaches account for a large percentage of impervious increase; top 10, or 0.134%, are 372ac or 15.9% total increase (tenth of a percent of all reaches are sixteen percent of impinc area); 99 of 7437 or 1.3% of reaches are 1000ac of 2300 total, etc.  
...
# Discussion
 ...
 - clarify that HRCD offers retrospective look at what happened within riparian corridor that was not necessarily regulated as critical area Fish and Wildlife Habitat Conservation Areas FWHCA; main takeaway is that additional protection is warranted if controlling adverse effects of imperv is policy goal
