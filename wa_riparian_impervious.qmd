...
Though continued alteration is qualitatively certain, these studies demonstrate several of the factors that contribute quantitative uncertainty to the primary research question of "how much change in riparian impervious cover". In addition to the potential for variation due to alternative spatial dimensions of "riparian", the underlying hydrographic flowlines and waterbody areas have positional error, such that not all streams, rivers and buffers are 'where they should be', independent of 'how wide they should be'. But accepting hydrographic error and given a particular buffer formulation, landscape scale estimates have typically [usually? exclusively?] relied on categorical classifications of satellite imagery, usually at a pixel resolution corresponding to a 30m ground distance. Within an image at a certain point in time, substantial spatial variation is understood to occur within the 30x30m area of each pixel, prompting various methods to train the classification algorithms that translate continuous spectral values to categories and to constrain the changes among classes (CITES beyond those above?). 

The accuracy of estimates based on these methods is likely to increase where land cover is relatively homogeneous and change areas are large relative to pixel resolution. But much of the ongoing addition of impervious cover in Washington is concentrated within heterogeneous mosaics of multiple land covers and is thought to derive from numerous distinct parcels, with change extents frequently smaller than the 0.09ha (0.22 acres) of a single 900m2 pixel. Thus, the RDE incorporates estimates of impervious cover change based on
high resolution change detection (HRCD) processing of aerial imagery at 1m resolution (Pierce 2015). This approach combines automated classification of change polygons with extensive manual review and verification against before-after image pairs to generate a detailed dataset of alterations that is well-suited to geospatial intersection with the complex shapes of riparian networks. The effort to generate the RDE provided the resources needed to integrate the HRCD data with a statewide polygon tessellation of riparian corridors, thereby enabling this novel analysis of high resolution change at a statewide scale.


# Methods 

## High resolution detection of impervious cover

Land cover changes were measured following the basic methodology outlined in Pierce (2015), in which automated processing of sequential aerial imagery generated candidate polygons that were then manually reviewed for accuracy and categorical attribution. 

**Ken P: do you have a 'ready to go' further description of the impervious detection methods?**
...
**How much additional info on the polygon intersection and geodatabase creation? It's implicit in the Intro; could move, expand here, or something else?**
