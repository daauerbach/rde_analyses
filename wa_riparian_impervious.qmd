...
These studies demonstrate several of the factors that contribute quantitative uncertainty to the primary research question of "how much has riparian impervious cover changed?". In addition to the potential for variation due to alternative spatial dimensions of riparian buffers (e.g., from 10m to 200m), the underlying hydrographic flowlines and waterbody areas have positional error, such that not all streams, rivers and buffers are accurately located and may differ among studies. But accepting hydrographic error and given a particular buffer formulation, landscape scale estimates have also typically relied on categorical classifications of satellite imagery, usually at a pixel resolution corresponding to a 30m ground distance. Within an image at a certain point in time, substantial spatial variation is understood to occur within the 30x30m area of each pixel, prompting various methods to constrain the changes among classes based on algorithms that translate from continuous spectral values. 

The accuracy of estimates based on these methods is likely to increase where land cover is relatively homogeneous and change areas are large relative to pixel resolution. But much of the ongoing addition of impervious cover in Washington is concentrated within heterogeneous mosaics of multiple land covers and is thought to derive from numerous distinct parcels, with change extents frequently smaller than the 0.09ha (0.22 acres) of a single 900m2 pixel. This analysis therefore uses estimates of impervious cover change based on high resolution change detection (HRCD) processing of aerial imagery at 1m resolution (Pierce 2015). The HRCD approach combines automated classification of change polygons with extensive manual review and verification against before-after image pairs to generate a detailed dataset of alterations. The integration of HRCD data with a statewide polygon tessellation of riparian corridors enables this novel analysis of high resolution change at a statewide scale.
...
## Riparian Data Engine: Intersection of change with riparian corridor
...
Values of the HRCD-based increase in impervious cover were tabulated statewide and per-county, treating county boundaries as *geographic* units that contain multiple jurisdictions (i.e., *not* as the jurisdictional boundaries of individual county governments).
...
Annual county-level housing units were accessed from the Washington Office of Financial Management (OFM) Data and Research [April 1 Postcensal Estimates](https://ofm.wa.gov/washington-data-research/population-demographics/population-estimates/historical-estimates-april-1-population-and-housing-state-counties-and-cities). These data 
...
wa_imp <- rde_cnty |> 
  ...
  mutate(
    imp_pct_reach_ac = nonfstry_impv_inc_ac / reach_ac,
    #per KP, HRCD 2011:2017 NAIP is summer to summer
    ha_y = 0.4*(nonfstry_impv_inc_ac / 6),
    ac_y = nonfstry_impv_inc_ac / 6 
  ) 
```
...
The results presented here are based on the full "riparian corridor" constituted by the sum of the RMZ, EOW, and floodplain polygons associated with each RDE reach unit. Stratification by county refers to the area within geographic boundaries rather than the jurisdictional extent of county governance. Values within these geographic boundaries reflect various combinations of federal, tribal, state, and incorporated municipal regulatory regimes, and are not due to the actions of a single managing authority.

```{r sf_for_new_multipanel}
list(
  #range(rde_cnty_sum_impv$reach_ac)
  reach_ac = sf_cnty_rde |>  
    ggplot() + geom_sf(aes(fill = reach_ac), show.legend = T) + 
  ...
  est_pop = sf_cnty_rde |>  
    ...
    labs(subtitle = "2011 estimated population")
  ,
)

- reach_ac, swifd-ac, pop/housing (normalized to cnty_ac or reach_ac), imperv, tree loss

```


At the full statewide scale, `r round(0.4*wa_imp$nonfstry_impv_inc_ac)` hectares of new impervious cover were added during 2011-2017 (table NNNN; `r round(wa_imp$nonfstry_impv_inc_ac)`). This corresponded to `r wa_imp$imp_pct_reach_ac` of the `r round(0.4*wa_imp$reach_ac)` total reach hectares, and annual increase of `r wa_imp$ha_y` ha (`r wa_imp$ac_y` acres).

```{r gt_statewide_ac_year}
wa_imp |> 
  gt(caption = "2011-17 HRCD Impervious Increase within RDE Extent") |> 
  fmt_number(decimals = 0) |> 
  fmt_percent(columns = "imp_pct_reach_ac")
...
Most individual changes were small, with nearly 95% less than 0.4ha (1ac) and a median increase of `r 0.4*0.06` ha (0.06ac, as the intersection of HRCD and RDE polygons), considerably less than the 900m^2 area of a single 30m pixel. This pattern was broadly consistent across portions of the state with more and less total increase (fig NNN map/column panels). 
...
Total increases varied considerably among county boundary extents, with the largest additions concentrated in more heavily urbanized portions of the state, particularly central and south Puget Sound. Congruent with expectations, increases were greatest in proximity to the large and rapidly growing metropolitan areas of Seattle, Tacoma and Vancouver in western Washington and Spokane and Tri-Cities in the east.
...
Examining reach-area normalized change versus total aggregate increase highlighted differences in the relative density of new impervious cover (figure NNN scatter panels). Higher per-unit area rates in several instances followed from similar impervious additions that occurred within smaller overall riparian corridor extents.
...
Similarly, when examined relative to total housing unit change during the same 2011-2017 time period, considerable differences in the reach-area normalized impervious increase were evident between county boundaries. This comparison indicates only general patterns in the configuration of land cover conversion because changes included various anthropogenic causes such as roads, warehouses, etc., and were not restricted to residential construction. Nonetheless, the variation in this relationship highlights the potential for housing gains to come at quite divergent costs in riparian condition.
...
rde_cnty_sum_impv |> 
  ...
  geom_smooth(method = "lm") +
  wacolors::scale_fill_wa_c("volcano", reverse = T, na.value = "pink", aesthetics = c("fill","color")) +
  scale_x_continuous(breaks = seq(0, 70000, by = 10000), labels = scales::comma) +
  #scale_x_log10(labels = scales::label_log()) + 
  labs(
    x = "OFM 2017-2011 total housing unit change", 
...
Finally, at a statewide scale, the per-unit-area impervious increase differed sharply relative to the distribution of Pacific salmon as recorded in SWIFD, particularly for portions of the riparian network associated with Chinook (figure NNN swifd-columns). Although total aggregate increases were nearly equivalent between non-salmon and Chinook-associated reaches (427 vs 440 ha, 1068 vs 1100 ac), the overall area of the former was roughly an order of magnitude larger, resulting in a much greater rate of change in the fish-occupied portions. 

```{r gg_nonfstry_impv_inc_ac_swifd}
...
# }/{
  rde_cnty_swifd |> 
    ggplot(aes(swifd_chin, nonfstry_impv_inc_ac_per_100ac)) +
    geom_col(aes(fill = swifd_chin), show.legend = F) + 
    geom_text(
      aes(label = paste('total ac.:', round(nonfstry_impv_inc_ac))),
      nudge_y = -0.005
    ) +
    scale_fill_manual(values = pal_swifd_chin) +
    labs(x = "SWIFD status") 
...
We analyzed high resolution land cover change data within an ecologically-grounded riparian corridor to quantify the magnitude of new impervious area in Washington state during the early part of the 21st century. A statewide estimate of `r round(0.4*wa_imp$nonfstry_impv_inc_ac)` (`r round(wa_imp$nonfstry_impv_inc_ac)` acres) during the period from 2011 to 2017 represented `r round(wa_imp$nonfstry_impv_inc_ac / wa_imp$reach_ac, 5)`% of the total assessed extent, which encompassed a SPTH200 RMZ and adjacent floodplains. This estimate might be viewed simultaneously as a small portion of the entire riparian corridor and as a large new addition to an ecohydrologically sensitive zone in watersheds already subject to extensive alteration.
  
## [Caveats]

Riparian buffer dimensions that influence ongoing permitting, timber harvest, and other management actions are far from uniform, with a range of legally stipulated values in state regulations and local critical area ordinances (Folkerts et al. 2020), in addition to the heterogeneity in realized on-the-ground conditions within any particular jurisdiction (Swartz et al 2024). The 200-year Site Potential Tree Height standard in WDFW guidance has policy influence as a formulation of best available science, but it does not necessarily determine the footprint of protected areas. The full riparian corridor examined in this analysis does not represent any specific jurisdiction and is likely to encompass what are frequently narrower or smaller management zones. Accordingly, these results are likely an over-estimate relative to the (highly varied) mosaic of currently binding distances. However, the corridor in this assessment is likely to have hydrologic and ecological significance to in-channel conditions regardless of its legal circumstances, and the RDE RMZ tessellation is itself based on underlying hydrography (NHD) that is missing extensive lengths of the smaller, sometimes intermittent or ephemeral components of drainage networks that significantly contribute to water quantity and quality (Brinkerhoff et al 2024). These factors increase the likelihood that this analysis under-estimates the overall magnitude of functional impacts to aquatic ecosystems due to new impervious cover. 

In addition to the hydrographic error and that in the site index values on which SPTH200 dimensions are based, some residual error persists in the high resolution change polygons despite their manual review. This composite set of factors prevents a straightforward quantification of uncertainty, but as a coarse indication of possible bounds on these estimates, imposing arbitrary 25% factors yields a range from `r 0.4*0.75*rde_cnty_swifd$nonfstry_impv_inc_ac[3]` to  `0.4*1.25*rde_cnty_swifd$nonfstry_impv_inc_ac[3]` in the total hectares of 2011-17 new impervious cover in Chinook-associated reaches. Even the low end of this range raises questions about whether a "no net loss" standard can have been met: if hundreds of acres of impervious cover are not being removed each year, what actions have offset those that are being added?

Moreover, the addition of upland imperviousness outside of this or any other assessed corridor from 10m to 100m is highly likely to affect riparian and in-channel conditions (i.e., a 30m "buffer" is unlikely to have the same function as part of a continuous forest versus as a distinct strip adjacent to converted land). Beyond site-specific slope and soils factors, the concurrent surrounding watershed context has inevitably shaped our understanding of the functional dimensions that underlie buffer formulations. But the applicability of distance-decay curves likely warrants review as increasing imperviousness continues to alter flow paths (e.g., away from saturation excess and towards infiltration excess dynamics). While an SPTH standard allows for some flexibility to avoid prescribing a single critical area dimension, the basic concept may be invalidated where upland surface alteration has rendered even some multiple of SPTH (2x or 5x etc.) insufficient to perform all desired functions.


- east vs west considerations?


## [Connections]

These results were broadly in agreement with prior studies at coarser resolutions and/or smaller scales.
  - Lazarus
  - StreamCat rp100 NLCDurbhimd 11700/18 = ~650ac/y?

## Next steps

  - Process 2021 1m imperv CCAP/Ecopia to SPTH200?
...
- RDE/HRCD
  ...
  - independently verify that HRCD is exclusively 2011-17?
...
Washington is a leader in careful land and water use management, striving to encourage vibrant human communities without compromising the natural systems that make them possible. Yet at a statewide scale, high resolution data indicate that each year hundreds of acres are still being converted to impervious surface in ecohydrologically sensitive locations.
...
```{r objects_streamcat_nlcd, include=F, eval=F}
