...
#maybe not the most sensible way to viz, better as lm-est-vs-lm-est or similar?
inner_join(
  sc_nlcd_ac_y_cnty
  ,
  ofm_housing_cnty
  ,
  by = c("cnty", "nlcd_year" = "year")
) |> 
  filter(nlcd_class == "urbhi", aoi != "cat") |> 
  filter(str_detect(cnty, "King|Pierce|Snoho|Spoka|Clark|Kitsap|Yakim|Thurst|Whatc")) |> 
  ggplot(aes(total_housing, ac, color = cnty, group = cnty)) +
  geom_point(size = 0.5) +
  geom_line() +
  facet_wrap(~aoi, ncol = 1, strip.position = "right", labeller = label_wrap_gen(multi_line = F), scales = "free") +
  labs(subtitle = "StreamCat NLCD urbhi cover relative to OFM housing est. 2001-2019\nwithin (lower, rp100) and outside 100m riparian corridor (d_cat_rp100)")
...
The full set of whole-WRIA reach tables were compiled from the public web application.

```{r build_rde_wria_library, eval=FALSE}
# #first DL csv
# walk(
#   #1:30,
#   31:62,
...
#qaqcing Skagit to better understand how much of this change may be in floodplain
