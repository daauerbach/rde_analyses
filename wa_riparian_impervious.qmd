...
The results presented are based on the area (acreage) of the full "riparian corridor" constituted by the sum of the RMZ, EOW, and floodplain polygons associated with each RDE reach unit. Stratification by county refers to the area within geographic boundaries rather than the jurisdictional extent of county governance. Values within these geographic boundaries reflect various combinations of federal, tribal, state, and incorporated municipal regulatory regimes, and do not follow from the actions of a single managing authority.
...
The size distribution of individual increases showed that most changes were small, with nearly 95% less than 0.4ha (1ac) and a median of `r 0.4*0.06` ha (0.06ac), considerably less than the 900m^2 area of a single 30m pixel. This pattern was broadly consistent across portions of the state with more and less total increase (fig NNN). 
...
Examining reach-area normalized versus total increase highlighted differences in the relative density of new impervious cover, with some instances experiencing similar additions within half or a quarter of the overall riparian extent.
...
Patterns of reach-area normalized increase versus total housing unit change also showed that comparable housing gains were associated with considerable differences in new riparian impervious cover.
...
Finally, at a statewide scale, the distribution of reach-area normalized impervious increase was sharply divergent between portions of the riparian network with any salmon distribution and particularly with Chinook.
...
# Discussion
  ...
  We analyzed high resolution land cover change data within an ecologically-grounded riparian extent to quantify the magnitude of new impervious cover in Washington state in the early part of the 21st century. A statewide estimate of `r round(0.4*wa_imp$nonfstry_impv_inc_ac)` (`r round(wa_imp$nonfstry_impv_inc_ac)` acres) during the period from 2011 to 2017 represented `r round(wa_imp$nonfstry_impv_inc_ac / wa_imp$reach_ac, 5)`% of the assessed total area, which encompassed a SPTH200 RMZ and adjacent floodplains. 
  
  This estimate might be viewed as both small, considered as a portion of the total riparian corridor, and large, as new conversion frequently occurring in the context of existing watershed alteration.
  
  found the answer to be both "a little", in that individual additions were mostly less than 1ha, and "a lot", in that the cumulative total of many small changes was still appreciable.

In agreement with prior studies at smaller scales and coarser resolution, 

StreamCat rp100 NLCDurbhimd 11700/18 = 650ac/y?

- examined extent is not jurisdictional anywhere; buffer dimensions in accordance with those of the SPTH200 standard in guidance are not widely implemented
