...
need to rebuild against: https://api-ripariandata.wdfw.wa.gov/
samke key, confirmed 8/5/25 can read_csv for wrias
could redo by counties via https://api-ripariandata.wdfw.wa.gov/counties/{countyName}/reaches/csv
but wrias is more convenient to loop over numeric index

walk(
  1:3, #31:62, #1:62, 
  
  ~httr2::request(paste0(
    # 'https://api-riparis.esa-prod.sitkatech.com/wrias/',.x,'/reaches/csv'
...
```{r streamcat_nlcd_setup_sf_objects}
library("tidyverse", quietly = T)
library("sf")
library("patchwork")
library("gt")
...
-starting object with ac, and urb_ and tree_?
