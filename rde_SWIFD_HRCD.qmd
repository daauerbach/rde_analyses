...
# #but also many scattered streamriver and artificalpath
# #statewide but large concentrations in yakima region
# sf_flw |> 
#   #as_tibble() |> count(ftype) |> mutate(pct = n/sum(n))
#   anti_join(distinct(as_tibble(sf_cat)), by = "comid") |> #9067
#   #ggplot() + geom_sf() + geom_sf(data = sf_wrias, fill = NA, color = "red")
#   #as_tibble() |> count(ftype) |> mutate(pct = n/sum(n))
#   mapview()
# #cat with no flowlines are edge cases, a pothole depressional area SE of Okanagon plus a few other scattered Eastside
# sf_cat |>
#   anti_join(distinct(as_tibble(sf_flw), comid), by = "comid") |>#257
#   ggplot() + geom_sf() + geom_sf(data = sf_wrias, fill = NA, color = "red")
...
#starting object with cat and catrp100
sc_nlcd_cat <- readRDS(file.path(dir_data_common, "streamcat", "sc_wa_nlcd.rds")) |> 
  map(
    ~.x |> 
      rename_with(~str_remove(.,paste(c(2001, 2004, 2006, 2008, 2011, 2013, 2016, 2019),collapse = "|")) |> 
                    str_remove("pct")) |> 
      select(comid, nlcd_year, catareasqkm, catareasqkmrp100, ends_with("cat"), ends_with("catrp100"))
      ) |> 
  list_rbind() |> 
  inner_join(
    as_tibble(sf_cat) |> select(comid, wria, wria_nr_nm), by = "comid"
...
#treed riparian area: what about 'wdwet'?
#urblo and urbop are doing different things
sc_nlcd_cat_ac <- sc_nlcd_cat |> 
  #filter(is.na(urbmdcatrp100)) |> distinct(comid)
...
sc_nlcd_cat_ac |> 
  ...
  select(comid, nlcd_year, wria, wria_nr_nm,
         starts_with("urb_") #urb_cat, urb_catrp100
         ) |> 
  #pivot_longer(-c(wria, comid, nlcd_year)) |> 
  #pivot_wider(names_from = nlcd_year, names_prefix = "nlcd_", values_from = value)
  ...
  ggplot() + 
    geom_segment(aes(
      ...
      color = d_cat - d_catrp100 #color = d_cat > d_catrp100 #color = wria
      ),
...
sc_nlcd_cat_ac |> 
  ...
  labs(
    ...
    subtitle = "StreamCat NLCD %confi+decid+mxfst",
    x = "whole local catchment",
...
#above is interesting but still not focusing the tree change on only those comids with +urban
#comids with non-zero urban cover in final NLCD year
#could threshold to larger %
...
pal_nlcd <- c("brown","darkgreen","yellow","green","yellowgreen","yellow","seagreen","green","blue","tan","grey10","grey30","grey60","grey80","seagreen")

sc_nlcd_cat_ac |> 
  filter(
    nlcd_year %in% c(2001, 2019)
    ,
    comid %in% comid_2019_urb_rp100
    ) |> 
