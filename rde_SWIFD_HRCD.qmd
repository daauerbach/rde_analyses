...
sc_wa_nlcd_rp100 <- readRDS(file.path(dir_data_common, "streamcat", "sc_wa_nlcd_rp100.rds"))
#wrangle to single tibble: 
#strip year from colnames, focus on urban 'high'+'medium', 4 tree categories 
sc_wa_nlcd_rp100 <- map(
  sc_wa_nlcd_rp100,
  ~.x |> 
    rename_with(~str_remove(.,paste(names(sc_wa_nlcd_rp100),collapse = "|")) |> 
                  str_remove("pct")) |> 
    ...
    mutate(
      urbrp100_cat = urbmdcatrp100 + urbhicatrp100,
      urbrp100_ws = urbmdwsrp100 + urbhiwsrp100,
      treerp100_cat = wdwetcatrp100 + mxfstcatrp100 + decidcatrp100 + conifcatrp100,
      treerp100_ws = wdwetwsrp100 + mxfstwsrp100 + decidwsrp100 + conifwsrp100
    ) |> 
    select(comid, nlcd_year, ends_with("_cat"), ends_with("_ws"))
  ) |> 
  list_rbind() |>
