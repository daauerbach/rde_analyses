...
sc_nlcd_cat_tree <- sc_nlcd_cat |> 
  mutate(
    ...
    tree_cat_ac = tree_cat * catareasqkm * 247.11,
    tree_catrp100_ac = tree_catrp100 * catareasqkm * 247.11
    ) 

#ignoring years
sc_nlcd_cat_tree |> 
  ggplot() + geom_boxplot(aes(tree_cat, fct_reorder(wria, tree_cat)), varwidth = T)
#ignoring years, contrasting rp100 wria-wide
sc_nlcd_cat_tree |> 
...
#ignoring years, riparian-vs-whole per-local diff
...
#starting to re-demo concept, redoing '+urbancat+treerp100'
#comids with non-zero urban cover in final NLCD year
#could threshold to larger %
comid_2019_urb <- sc_nlcd_cat |> 
