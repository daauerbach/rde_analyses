...
[beginning to draft something that combines rde_SWIFD_HRCD analyses with sc_ripcorr_nlcd, 
eventually probably a new file but keeping here for now]
...
This analysis therefore addresses the need for landscape scale quantification, with a primary focus on "urban" cover as an indicator of imperviousness. Impervious cover has statewide relevance in mesic (western) and semi-arid (eastern) regions, and is well-understood to be associated with ecohydrologic degradation. Specifically, we investigated the seemingly simple question "How much impervious surface has Washington state added to riparian zones in the 21st century, and how has that related to prior changes during the 20th century?"

Past research has indicated likely patterns without providing a clear answer to this question. Jones et al. (2010) considered late 20th century land cover change (1973-2003) at relatively coarse spatial resolution (60m) within a 180m buffer of NHD watercourses in several thousand sample blocks across the continental United States. They described a nationwide monotonic increase in the average riparian "urban %" over the assessed duration, and their results showed the Puget lowlands region had the largest decrease in "natural land cover" during the "mid 80s - early 90s" period (relative both to its change in other time periods and to all other regions nationally in that period).
...
Broadening to more watersheds, Bartz et al. (2015) found similar dynamics in an assessment of annual changes derived from a LandTrendr per-pixel analysis of 30m imagery within 100m or 200m buffers of salmon habitat geographic features. Within these assessed areas, they estimated that, by 2008, impervious cover in mainstem zones increased roughly 172ha (~425ac, 3%) from ~5820ha in 1986, and that tributary zones increased roughly 119ha (294ha, 5.5%) from ~2,153ha. However, they also reported evidence of a "flatter" rate of increase in urban cover across these and the other nearshore, estuary, and floodplain habitat areas examined, with the timing of inflections varying considerably among habitat types and subbasins. These authors were unequivocal that a legacy of degradation continues to affect Puget Sound habitats, but they also concluded that more recent land cover change may indicate slower growth in additional impairment, perhaps associated with a combination of cultural, economic and regulatory factors.
  ...
  - Lazarus 2023 thesis (different stratification: public-private, other exclusions)

terms and defs: 
 - rmz vs 'corridor', buffer etc.
 - impervious, urban, developed


...comparing the RDE with a simple, spatially-defined riparian corridor

...using readily-available datasets to examine broad patterns that span numerous jurisdictions as well as substantial hydroclimate and habitat variation. 
...
- Do we know any locations with unambiguous ground level de-paving/removal of impervious surface?
```{r elwha_reservoir_in_nlcd}
x<-filter(sf_flw, wria=="18"); y<-filter(sf_cat, wria=="18"); mapview::mapview(x)+mapview::mapview(y)
#lower dam and resvr: "23997060"
#upper mid resvr, could include several other lines: "23997114"

x<-filter(sf_flw, comid=="23997114") |> st_buffer(dist = 300)
y<-filter(sf_cat, comid=="23997114")
mapview::mapview(x)+mapview::mapview(y)
#yep. very nice. 
sc_nlcd |> 
  filter(comid=="23997114") |> 
  select(wria_nr_nm, cnty, comid, nlcd_year, ends_with("_ac"), -contains("urb_")) |> 
  pivot_longer(ends_with("_ac"), values_to = "ac") |> 
  mutate(
...
via Ken M: [Danielle Lazarus MS thesis, adv Withey](https://collections.evergreen.edu/files/original/ba7eb58e31f36d4592dadb2e962112724ca53982.pdf)
- nice intro, lit review; strong policy awareness; appreciate the 'positionality' statement
- statewide, NLCD + CDL at 50/100/SPTH200
- buffer creation looks similar to RDE tessellation (combo of line and poly NHD hydrography, also VSW); appear to have unioned individual segments before raster extracts/zonal stats
- reclassed to woody/shrubherb/devhimd/devloop and then dev/ag/'potential riparian' (tables 3&4, p92 of pdf, p74 of doc)
- table 7 lists buffer acreages by 'simple' & total classes; statewide SPTH of ~7.979m is > ~6.4m for rp100 estimate from 2001 despite tribal and openwater/ice exclusions, coincidence that '100ft' total ~6.139m (3.413W + 2.726E) is much closer?
- table 16 has 2019-2001 SPTH statewide dev at +11770ac (-1172pub + 12936priv) as hi+md+lo+op, with table 18 having himd vs loop split but only as pct; rp100 calc is 11870 = 406476 - 394606 = (17954+59037+133425+196060) - (14831+47626+131655+200494)
  - note table 18 devmh spth pct increases very similar across buffers (0.11%-0.13% west, 0.16%-0.17% east); assuming these are not pct change of pcts, but instead abs change of pcts, for rp100 I get ~0.226% (76991/6413939)-(62458/6413939)

seems that wider rp100 on shorter/fewer stream miles (NHDplus) is in the ballpark of narrower spth on longer/more hydro network...at least for urban/dev classes where upper network fringe hydrographic diffs less determinative?
```{r}
sc_nlcd |> 
  summarise(across(ends_with("rp100_ac"), sum), .by = nlcd_year) |> 
  select(nlcd_year, contains("urb")) |> 
  filter(nlcd_year %in% c(2001,2019)) |> 
  pivot_longer(-nlcd_year) |> 
  pivot_wider(names_from = nlcd_year) |> 
  mutate(d = `2019`-`2001`)
```


Powell et al. 2008 Quantification of impervious surface in the Snohomish Water Resources
Inventory Area of Western Washington from 1972–2006
  - Landsat, detailed methods on multistep spectral and time series processing of ~30m, created manual riparian masks from orthophoto as well as using UGA polys to apply location specific rules to images to reach 'custom' classes, with imperv related to whether "no/low veg" was within UGA/ag/rip/frst masks, Fig5 shows distribs of components; seems likely to be a high quality estimate
  - but results are WRIA-wide not for riparian exclusively, e.g. Fig7, hard to tell exactly how any imperv was treated within the riparian manual mask
  - "impervious surface in the Snohomish WRIA increased by 255% over 34 years, from 3285 ha in 1972 to 11,652 ha in 2006"
  - interesting comparison to 2001 NLCD, noting 11893ha NLCD vs their 10568; note NLCD higher in urb, theirs higher in 'exurb'; Fig9 further distinguishing urban/exurban diffs by processing; note also temporal rules to decrease illogical pixel switching 

Jones KB, Slonecker ET, Nash MS, Neale AC, Wade TG, et al. (2010) Riparian habitat changes across
the continental United States (1972–2003) and potential implications for sustaining ecosystem ser-
vices. Landscape Ecol 25: 1261–1275.
  - 1973-2003 in 5 periods for 10/20km blocks within HUCs, 30m coarsened to 60m from Landsat+NAIP, NLCD baseline + manual, 180m buffers on 1:24K NHD
  - Fig4 shows per-period increases in "%Urban", no helpful breakdown to WA/PNW
  - Fig6 shows "eco2" PS-lowlands loss of natural strongest in late70-80s and mid80-early90s (largest mean decline of all ecoreg)

Bartz, K. K., M. J. Ford, T. J. Beechie, K. L. Fresh, G. R. Pess, R. E. Kennedy, M. L. Rowse, and M. Sheer. 2015. Trends in developed land cover adjacent to habitat for threatened salmon in Puget Sound, Washington, U.S.A. PLoS One 10:1–22.
  - Puget Sound only (and not Kitsap, HC, fair bit of JDF), LandTrendr Landsat 30m 1986-2008
  - 100m on various forms/habitat types, mainstem >25m bfw distinguished from tribs 5-25m bfw
  - unclear _exactly_ what "developed" was here since it comes from randomforest trained on NLCD2001, but no uncertainty in what the "associated most strongly with hi/lo" means other than 64%/69% accuracy for dev/evergreen in manual random validation, and 78%/90% for dev/no-dev: "LandTrendr-stabilized images were used to develop yearly land cover maps based on training information derived from the 2001 National Land Cover Database (NLCD) land cover maps. Because not all of the 16 original land cover classes in the NLCD maps were statistically separable, they were first aggregated into 7 classes that were distinguishable from spectral data alone: open water, snow/ice, barren land, deciduous forest, evergreen forest, herbaceous, and developed. The LandTrendr “developed” class is associated most strongly with the medium-, and high-intensity developed NLCD classes, defined as 50–79%, and 80–100% impervious, respectively. Stabilized imagery from 2001 was linked with the aggregated NLCD land cover at a random sample of locations across the Puget Sound region, and the RandomForests classification algorithm was used to model the relationship between spectral data and the land cover classes. Once established for 2001, this model was then applied to stabilized imagery from all other years between 1986 and 2008."
  - notably report not just flattening increases but actually _decreasing_ developed areas in some basins/mainstems/tribs, esp after breakpoints, see Fig 8, esp Nisq and Desch mainstem;
    - explain as possibly due to vegetation maturation/growth over no-longer-new impervious that alters spectral signature (i.e., an artifact rather than a true ground-level decrease), but note possibility of being partially due to floodplain/mainstem restoration actions...

Swartz et al 2024 Quantifying the Variability of “Fixed-Width” Buffers on Harvested Lands in Western Oregon and Washington
  - nice study concept, good intro; most apt for discussion?

[superseded by Powell et al]
Alberti, M., R. Weeks, and S. Coe. 2004. Urban land-cover change analysis in central Puget Sound. Photogrammetric Engineering & Remote Sensing 70:1043–1052
...
[emailed]
Me, curious what ChatGPT would offer, not because I necessarily expected it to be correct, but exactly because I was guessing it would help to reveal that it’s a harder problem than it feels like it should be. I was not prepared for just how nicely it frames the research:
 
How much impervious surface has Washington state added to riparian zones in the 21st century, and how has that compared to the prior phase of industrialization during the 20th century?
 
Unfortunately, I was unable to locate any peer‑reviewed or government-confirmed estimates quantifying how many acres (or square miles) of impervious surfaces have been added specifically within Washington State’s riparian zones in the 21st‑century versus the 20th century. The data simply haven’t—or at least not in a way readily accessible to search.
🧾 What We Do Know
1. Impervious surface growth in the Puget Sound region
Since the 1990s, urban and suburban development around Puget Sound has driven a marked rise in impervious areas (roads, rooftops, parking lots), which substantially increase stormwater runoff and pollutant loads to rivers and streams—impacting salmon and other riparian-dependent species (Reddit).
State strategies now aim to reduce impervious cover in key urban basins below 30% by 2031 through green infrastructure and depaving projects (Department of Natural Resources).
2. Historical land-use transformation—20th century
During the 20th century, riparian zones were heavily transformed through industrial and milling operations, channel straightening, logging roads, floodplain filling, and agricultural conversion—often without quantifying impervious cover per se, but drastically altering hydrology (e.g. North Creek at UW Bothell) (Wikipedia).
Forestry practices concentrated riparian logging and road construction across river corridors—though the exact figure of impervious surface (gravel roads, skid trails) throughout the 1900s isn’t tracked quantitatively in published sources (Wikipedia).
🔍 Comparison: Why quantitative figures are elusive
Riparian buffer regulations evolved over time (e.g. WAC riparian management zones established by 1999) but did not mandate systematic tracking of impervious cover additions within those zones (app.leg.wa.gov).
Impervious-surface monitoring typically occurs at watershed or basin scales—not isolated to riparian strips. Most urban stormwater studies focus on broader drainage areas.
Earlier industrial-era records prioritized timber harvest volumes and land clearance, not area of paved or built surfaces along streambanks.
🚦 Direction for more precise information
If you're looking to obtain more exact figures, the following avenues might help:
Washington Department of Ecology or regional stormwater monitoring reports that break impervious cover contributions by subbasin—perhaps isolating riparian margins.
County or city-level GIS analyses, potentially available through King County, Snohomish, Pierce, or other land‑use planning departments.
USGS or EPA studies that map changes in stream‑adjacent land cover between circa 2000 and present; these might allow you to extract riparian‑zone impervious share.
✅ Bottom line summary
There is no publicly available estimate quantifying the total added impervious surface area specifically within riparian zones of Washington State in the 21st century.
Compared to the broader-scale industrialization and logging, channel modification, and floodplain filling of the 20th century, it's likely that the absolute impervious increase within riparian corridors in recent decades is smaller, but still environmentally significant due to urbanization and stormwater runoff impacts.
Without targeted GIS or environmental modeling studies, a precise numerical comparison remains unavailable.
If you’d like, I can help you identify specific agencies or datasets—or suggest how to commission a GIS analysis to fill this gap.


via Ken M, same prompt to ScholarGPT:

The expansion of impervious surfaces in Washington State's riparian zones over the 21st century has been a continuation and intensification of trends that began during the 20th-century industrialization phase. The 20th century saw rapid urbanization driven by industrial development, leading to significant increases in impervious surfaces such as roads, parking lots, and buildings, which fragmented riparian habitats and disrupted hydrological functions. In contrast, the 21st century has been characterized by sprawling suburban development, more advanced land-use practices, and regulatory interventions, though the cumulative impact of impervious surface area continues to grow.
Key Findings from Research:

Booth (2000) examined the correlation between forest cover, impervious surfaces, and urbanization impacts in King County, Washington. The study found that even small increases in impervious surfaces within riparian buffers led to measurable hydrological and ecological impacts, with urban sprawl post-1980s contributing significantly to these changes.

Gregory et al. (1997) highlighted the transition of riparian management strategies from unregulated industrial expansion in the 20th century to more controlled urban growth with environmental considerations in the 21st century. However, these policies have struggled to halt cumulative impervious surface increases.

Yeakley & Hughes (2013) in "Wild Salmonids in the Urbanizing Pacific Northwest" emphasized how urban expansion in riparian zones increased significantly from the 1980s to the present, affecting salmonid habitats. The impervious surface impact was more diffuse but still severe in the 21st century due to suburban sprawl patterns.

Sweeney & Blaine (2007) pointed out that while 20th-century industrialization cleared riparian zones for agriculture and urban centers, the early 21st century has seen urbanization shift towards watershed-wide impacts with smaller but more widespread impervious additions.

[This is inaccurate, study was in TN]
Hall & Hossain (2020) analyzed impervious surface growth through remote sensing, showing that riparian zones continue to lose pervious surfaces at alarming rates in urbanizing areas of Washington State, driven more by diffuse suburban and exurban development rather than concentrated industrial zones.

Kaushal et al. (2015) described how urban evolution from the Industrial Revolution to present resulted in increasing decoupling of streams from their riparian buffers due to impervious surface growth. Washington State's riparian areas are no exception, facing "impervious encroachment" well into the 21st century.

Ricker et al. (2012) documented historic land use shifts, illustrating that impervious surfaces began dramatically impacting riparian soil functions by mid-20th century, while the 21st century has seen a diversification in impervious encroachment patterns rather than outright expansion rates.
...
Booth, D.B., 2000. Forest cover, impervious-surface area, and the mitigation of urbanization impacts in King County, Washington. University of Washington ResearchWorks Archive. Available at: https://digital.lib.washington.edu/researchworks/bitstream/handle/1773/19552/Forest%20Cover.pdf?sequence=1 [Accessed 4 August 2025].
Gregory, S.V., 1997. Riparian management in the 21st century. In: S.V. Gregory, ed. Streamside Management: Forestry and Fishery Interactions. Washington, DC: Island Press, pp.69–85. Available at: https://books.google.com/books?id=XNiq_KJ-zpgC&pg=PA69 [Accessed 4 August 2025].
Yeakley, J.A. and Hughes, R.M., 2013. Global and regional context of salmonids and urban areas. In: J.A. Yeakley and R.M. Hughes, eds. Wild Salmonids in the Urbanizing Pacific Northwest. New York: Springer, pp.11–26. Available at: https://scholar.archive.org/work/hvsimbwx2nclvmgcpb7jhau2hu/access/wayback/http://www.springer.com/cda/content/document/cda_downloaddocument/9781461488170-c1.pdf?SGWID=0-0-45-1428008-p175373186 [Accessed 4 August 2025].
Sweeney, B.W. and Blaine, J.G., 2007. Resurrecting the in-stream side of riparian forests. Journal of Contemporary Water Research & Education, 136(1), pp.17–27. Available at: https://nmwrri.nmsu.edu/footer_pages/nm-wrri-library-database-files/wrri-library-pdfs/wrrilibrary8/008783.pdf#page=21 [Accessed 4 August 2025].
Hall, J. and Hossain, A.K.M.A., 2020. Mapping urbanization and evaluating its possible impacts on stream water quality in Chattanooga, Tennessee, using GIS and remote sensing. Sustainability, 12(5), p.1980. Available at: https://www.mdpi.com/2071-1050/12/5/1980 [Accessed 4 August 2025].
Kaushal, S.S., McDowell, W.H., Wollheim, W.M., et al., 2015. Urban evolution: The role of water. Water, 7(8), pp.4063–4087. Available at: https://www.mdpi.com/2073-4441/7/8/4063 [Accessed 4 August 2025].
Ricker, M.C., Donohue, S.W. and Stolt, M.H., 2012. Development and application of multi-proxy indices of land use change for riparian soils in southern New England, USA. Ecological Applications, 22(5), pp.1474–1491. Available at: https://www.researchgate.net/profile/Michael-Zavada-2/publication/225051164_Development_and_application_of_multi-proxy_indices_of_land_use_change_for_riparian_soils_in_southern_New_England_USA/links/5627ff9008ae518e347b3173/Development-and-application-of-multi-proxy-indices-of-land-use-change-for-riparian-soils-in-southern-New-England-USA.pdf [Accessed 4 August 2025].
...
```{r need_work_comid_diff_histo, fig.height=6}
{
d |> 
  filter(d_out > 0) |> 
  ggplot() +
