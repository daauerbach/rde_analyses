...
Reach table does not include "impervious increase", but does not distinguish change agents, to differentiate, for example, "forestry" from "development"

```{r}
rde |> 
  summarise(
    across(
      c(riparian_ac, #rmz_ac, #all_land_cover_ac, all_tree_cover_ac,
...
rde |> 
  ...
  mutate(anth_pct = anth_chg_ac_sum / riparian_ac_sum) |> 
  arrange(wria, swifd_chin, t_imp_f_245) |> 
  filter(str_detect(t_imp_f_245, "impair")) |> 
  ggplot(aes(y = wria)) +
