...
rde <- rde |>
  ...
  mutate(
    t_imp_f = case_when(
      ...
      str_detect(t_imp, "Not") ~ NA_character_,
      .default = t_imp
    )
  )
```
...
rde <- rde |>
  mutate(
    swifd_chin = case_when(
      str_detect(swifd_salmon_distribution, "Chin") ~ "salmon_incl_chin",
      str_detect(swifd_salmon_distribution, "Chin", negate = T) ~ "salmon_not_chin",
      is.na(swifd_salmon_distribution) ~ "not salmon"
    )
  )
```
...
The acreage and average tree canopy cover within the RMZ corridor can be summarized according to these strata.
...
rde_st <- rde |> 
  ...
  ##sort 'grand percents' across wrias, not interesting?
  #mutate(across(ends_with("ac_sum"), list(pct = ~./sum(.)))) |> 
  
#  mutate(rmz_ac_pct_temp_in_swifd = rmz_ac_sum / sum(rmz_ac_sum), .by = c(wria, swifd_any)) |> 
  
  mutate(riparian_ac_pct_swifd_in_temp = riparian_ac_sum / sum(riparian_ac_sum), .by = c(wria, t_imp_f)) |> 
  arrange(wria, t_imp_f, swifd_chin)
```

# Visualizations

**UNFINISHED - should probably switch to 303d categories within swifd_chin levels (and functionalize plots etc.)**

```{r pal_swifd_chin, include=FALSE}
#just moving quickly
pal_swifd_chin <- c("not salmon" = NA, "salmon_incl_chin" = "salmon", "salmon_not_chin" = "tan")
```
...
```{r}
rde_st |> 
  filter(as.integer(as.character(wria)) < 26, is.na(t_imp_f)) |> 
  mutate(
  ...
  labs(x = "<----- no salmon                salmon ----->", y = NULL, 
       title = "Relative proportion of RMZ corridor by SWIFD Chinook presence for 'Not Assessed' 303d status",
...
```{r}
rde_st |> 
  filter(as.integer(as.character(wria)) < 26, t_imp_f == "3") |> 
  mutate(
    riparian_ac_pct_swifd_in_temp = if_else(
      swifd_chin == "not salmon", 
      -1*riparian_ac_pct_swifd_in_temp, 
  ...
  labs(x = "<----- no salmon                salmon ----->", y = NULL, 
       title = "Relative proportion of RMZ corridor by SWIFD Chinook presence for 303d 'insufficent data' stream temp",
       subtitle = "Darker green indicates higher average tree cover")
...
```{r}
rde_st |> 
  filter(as.integer(as.character(wria)) < 26, t_imp_f == "1") |> 
  mutate(
    riparian_ac_pct_swifd_in_temp = if_else(
      swifd_chin == "not salmon", 
      -1*riparian_ac_pct_swifd_in_temp, 
  ...
  labs(x = "<----- no salmon                salmon ----->", y = NULL, 
       title = "Relative proportion of RMZ corridor by SWIFD Chinook presence for 303d 'not impaired' stream temp",
       subtitle = "Darker green indicates higher average tree cover")
...
```{r}
rde_st |> 
  filter(as.integer(as.character(wria)) < 26, t_imp_f == "2") |> 
  mutate(
    riparian_ac_pct_swifd_in_temp = if_else(
      swifd_chin == "not salmon", 
      -1*riparian_ac_pct_swifd_in_temp, 
  ...
  labs(x = "<----- no salmon                salmon ----->", y = NULL, 
       title = "Relative proportion of RMZ corridor by SWIFD Chinook presence for 303d 'at risk/concerns' stream temp",
       subtitle = "Darker green indicates higher average tree cover")
...
Showing that the large majority of reaches/RMZs with category 4 or 5 impairment designations are also salmon-occupied reaches, with many also Chinook reaches.
