...
Aggregating to statewide totals, higher intensity urban cover has steadily increased within both local catchments and the `rp100` 100m riparian corridor at similar rates since 2001. Annualizing from a basic OLS fit of $$ acres_{urban} \sim year_{NLCD} $$, suggests that, on average, approximately 179 and 655 acres per year of high and medium intensity urban cover have been added within 100m of WA watercourses. Not all of this increase is necessarily due to losses of natural vegetation cover, though, in conjunction with the 95 acre/year increase in the "low" class, these increases substantially exceed the 264 ac/y decrease in urban "open".
...
However, considerable variation among WRIAs is evident in the overall magnitude and relative rates of higher intensity urban cover change within the riparian corridor relative to the encompassing local catchment area.
...
}+plot_layout(nrow = 1) + plot_annotation(title = "Estimated annual urban cover change 2001-2019")
```
