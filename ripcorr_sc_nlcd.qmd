...
#then second inner_join against sf_flw to get common set across all 3
...
sc_nlcd <- readRDS(file.path(dir_data_common, "streamcat", "sc_wa_nlcd.rds")) |> 
  ...
  inner_join(as_tibble(sf_flw) |> distinct(comid), by = "comid") |> 
...
The spatial and proportional distributions of land cover classes in 2001 follow expectations, with more intense urban cover types concentrated in the western portion of the state, particularly in central Puget Sound, and isolated higher cover corresponding to Spokane, Yakima, and Vancouver. A sum of 'forest' cover types (conifer, deciduous, and mixed) captures the large areas of commercial, state, and national forest in the Olympics and Cascades as well as the eastern Blue Mountain and Colville regions.
...
#catareasqkm includes catareasqkmrp100
  ...
  grand_summary_rows(columns = c("cat","rp100"),fns = ~sum(.), fmt = ~fmt_number(., decimals = 0)) |> 
...
*this is now deducting rp100_ac from cat_ac, but need to *
Aggregating to statewide totals, the higher intensity urban cover classes ... within ... local catchments and the `rp100` 100m riparian corridor. 

- abs areas smaller in rp100 but appear to be roughly similar rates, proportional to overall area; earlier simple bar plot was misleading due to abs-area y-axis scale, this view suggests no appreciably lower urb increase within rp100
...
however, considerable variation among and within wrias is evident in the relative urban cover increases...

```{r ggwria_ac_urb_nlcd_year}
sc_nlcd |> 
  summarise(
    across(
      contains("urb") & ends_with("ac"),
  ...
  mutate(d_cat_rp100 = cat - rp100) |> 
  pivot_longer(c(cat, rp100, d_cat_rp100), names_to = "aoi", values_to = "ac") |> 
  arrange(wria, aoi, nlcd_year) |> 
  filter(aoi != "cat") |> 

  #filter(nlcd_class=="urbhi", aoi =="rp100") -> d; lm(ac~nlcd_year, d) -> d; broom::tidy(d); rm(d)
  filter(wria %in% wrias) |> 
  filter(wria %in% c("1","57")) |> 
  ...
  map(
    ~.x |> 
      ...
      scale_fill_manual(values = pal_nlcd[grep(unique(.x$nlcd_class), names(pal_nlcd))]) + 
      scale_y_continuous(labels = scales::comma) +
      facet_wrap(~wria+aoi, ncol = 2, labeller = label_wrap_gen(multi_line = F), strip.position = "top", scales = "free") +
        labs(y = paste("acres", unique(.x$nlcd_class)), subtitle = unique(.x$nlcd_class)) +
      theme(strip.text = element_text(hjust = 0.1))
  ) |> 
  wrap_plots(ncol = 1, guides = "collect")
...
```{r starting_refactor_arrowswarm}
...
```{r perwria_sum_d1901_col_by_nlcd_class}
sc_nlcd |> 
  filter(nlcd_year %in% c(2001, 2019)) |> 
